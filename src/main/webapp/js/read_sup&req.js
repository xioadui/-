...
$(document).ready(function () {
    $.get(URL, function (data) {
        ...
        /*获得供/需*/
        $('.basic #two').append(type);
        /*获得类别*/
        $('.basic #three').append(time);
        /*获得发布时间*/
        $('.text').append(text);
        /*获得供需详细描述*/
    })
...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < len; i++) {
                $('.comment_message').append('<nav class="someone"><p class="some_name">' + data.entName[i] + '</p>');
                /*获得评论者名称*/
                $('.comment_message').append('<span class="some_time">' + data.commentDate[i] + '</span>');
                /*获得留言时间*/
                $('.comment_message').append('<p class="some_message">' + data.commentContent[i] + '</p>	</nav>');
                /*获得留言内容*/
            }
        },
